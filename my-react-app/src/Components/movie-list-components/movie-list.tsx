  ...
  //amountPages(Math.ceil(filterDataMoviesList.length / 10))
    return (
        <div className="movie-cart__main">
          {
          sliceDataMoviesListForPages(filterDataMoviesList(dataMoviesList, currentDateRelease), currentPage).map((item, index) => 
            <MovieCartItem key={index} title={item.title} raiting={item.vote_average} imgUrl={item.poster_path} isAuth={isAuth} setModalAuthorizationActive={setModalAuthorizationActive}/>
          )}
      </div>  
    ) 
  }
